...
export class MazeMap implements Iterable<MapValue[]> {
  ...
  _init_check_wall(p: Point, lp: Point): boolean {
    const temp = this._init_get_walls(p, lp);
    if (!temp) return false;
    for (const t of temp) {
      if (this.getitem(t) != MapValue.wall) return false;
    }
    return true;
  }
  ...
  _init_map() {
    /**初始化地图时所用的初始点 */
    const inst_st = (this.inst_st = new Point(
      this.random.randint(0, this.row - 1),
      this.random.randint(0, this.column - 1)
    ));
    ...
    while (stack.length > 0) {
      ...
      const around_walls = this._init_get_walls(p, lp);
  ...
  solve(pos?: Point): Point[] {
    ...
    const queue = new Queue<[Point, Point | null, number]>(
    ...
    while (!p.equal_to(this.ed)) {
      ...
      [p, lp, step] = _temp;
      map_temp[p.x][p.y] = lp;
      const roads = this._solve_get_roads(map_temp, p);
  ...
  setitem(...args: [number, number, MapValue] | [Point, MapValue]) {
    if (args.length == 2) {
      const [p, value]: [Point, MapValue] = args;
      this.map[p.x][p.y] = value;
    } else {
      const [x, y, value]: [number, number, MapValue] = args;
      this.map[x][y] = value;
    }
  }
